      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className={`relative flex justify-between items-center bg-white/80 backdrop-blur-xl border border-white/20 shadow-lg rounded-2xl transition-all duration-300 ${scrolled ? 'h-16 px-6 shadow-xl' : 'h-20 px-8'}`}>
          ...
          <Link href="/" className="flex items-center gap-2 group">
            <div className="bg-green-600 p-2 rounded-xl text-white transform group-hover:rotate-12 transition-transform duration-300 shadow-md shadow-green-600/30">
              <Tractor size={24} />
            </div>
            <span className="text-green-800 font-extrabold text-2xl tracking-tight hidden sm:block">
              6S GreenTech
            </span>
          </Link>
          ...
          <div className="flex items-center gap-3">
            {user ? (
              <>
                ...
                <div className="hidden sm:flex items-center bg-gray-50 px-4 py-2 rounded-xl border border-gray-100">
                ...
                <button 
                  onClick={logout} 
                  className="text-sm font-bold text-red-500 hover:bg-red-50 hover:text-red-600 px-4 py-2.5 rounded-xl transition-all"
                >
                  Logout
                </button>
              </>
...
// Custom NavLink component with beautiful hover effect
function NavLink({ href, children }: { href: string; children: React.ReactNode }) {
  return (
    <Link href={href} className="relative group text-gray-600 font-semibold hover:text-green-700 transition-colors py-2">
      {children}
      <span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-green-600 transition-all duration-300 group-hover:w-full rounded-full"></span>
    </Link>
  );
}
